...
def selectElitePopulation(stocktickers, population, selectionRate, startdate, enddate, runId, epoch):
...
def runGA(pop_size, stocktickers, maxStocks, maxIterations, selectionRate, crossoverRate, mutationRate, 
    ...
    elite = selectElitePopulation(stocktickers, population, selectionRate, startdate, enddate, runId, 0)
    iteration=0 
    sharpeRatio = 0
    starttime = time.time()
    prevElite = elite[0]
    ...
    while (iteration < maxIterations):
        ...
        elite = selectElitePopulation(stocktickers, population, 0.5, startdate, enddate, runId, iteration)
    ...
    if (sharpeRatio < 2): #Unsatisfactory local maximum! Rerun the whole GA with a different initial population
        ...
        if (depth >=5):
           ...
           runGA(pop_size, stocktickers, maxStocks, maxIterations, selectionRate, crossoverRate, mutationRate,startdate, enddate,depth)
    else:
        endtime = time.time()
        print('Portfolio of stocks creation after all the iterations finished.\n')
        print('total time taken: ', endtime-starttime)
...
def index():
    if request.method == 'POST':
        ...
        runGA(10, stocktickers, 10, 40, 0.3, 0.6, 0.6, '2021-09-27', '2022-10-05',0)
